## 1.1.10 beta.2
- исправлена ситуации утечки ВПН трафика при отключении стандартного интерфейса ВПН
- когда стандартный интерфейс ВПН выключается Keenetic уничтожает маршрут по умолчанию в таблице маршрутизации, добавил добавление его при включении.

## 1.1.10 beta.1
- небольшие исправление ошибок

## 1.1.10 alpha.5
- убрал обновление белого списка через каждые 5 минут. У списка заменил время жизни на 3600, синхронизировал это время с кэшем dnsmasq
- убрал многочисленные пересоздание правил в iptables
- main/ipset обновил с main ветки. Убрал многочисленные запуски этого скрипта, оставил только запуски с модуля check
- изменен алгоритм инициализации таблиц, перенесено из ndm.fs в init.d, изменено имя стартового скрипта на "S55kvas" что бы запускался раньше dnsmasq 
- в зависимости от установки, добавлен новый пакет conntrack. Необходим для сброса зависших соединений в таблице отслеживания состояний (conntrack), при смене интерфейса выхода.
- заменил в настроках SSR_DNS_PORT на PROXY_PORT
- добавлена маска 0xd1000 для всех операций MARK и CONNMARK, чтобы не затрагивать другие метки.
- устранена утечка трафика вне ВПН. Введена проверка и DROP для пакетов, попавших на маркировку, но после прохождения цепочки не получил метки.
- убрал пересоздание правил при старте ВПН соединения, правила iptables не зависят от ip адреса интерфейса.
- теперь при добавлении или удалении хоста в белый список не перезапускается весь КВАС, только dns сервер.

## 1.1.10 alpha.4
- изменил имя таблицы ipset на KVAS_UNBLOCK
- изменил kvas-iface-add теперь он полностью создает файл интерфейсов со структурой id|system_name|type|description

## 1.1.10 alpha.3
- убрал скрипт 100-proxy-redirect при установке пакета, не нужно его по умолчанию ставить в nmd
- переименовал скрипт создания ipset таблицы при монтировании fs на 15-kvas-start.sh
- убрал очистку таблиц маршрутизации при закрытии VPN соединения, сайты которые должны ходить через VPN не будут ходить на прямую
- убрал несколько дублей создания ipset IPSET_TABLE_NAME

## 1.1.10 alpha.2
- исправил дублирование скриптов в iflayerchanged.d или ifstatechanged.d сейчас там только один скрипт 100-kvas-vpn
- переместил скрипт ndm в папку libs где ему и место
- Произведена замена файла hosts.list на kvas.list

## 1.1.10 alpha.1
- все глобальные переменные перенес в отдельный файл env.sh
- перенёс параметр версии и релиза из kvas.conf пока что в файл env.sh
- служебное. написал скрипт который создает ipk без необходимости установки sdk. (пока что еще не пробовал как он будет устанавливаться)
- изменил репозиторий на https://api.github.com/repos/carIod/kvas/releases 
- добавил новую комманду kvas vpn manual где вручную можно написать интерфейс которого нет в CLI но есть в Entware и который предоставляет VPN канал. (не тестировал) 
- заменил окончание строк с CRLF на LF
- исправлена некорректная вставка пароля ShadowSocks
- переделано получение версии OS, берется из поля "release"
- отключил определение ускрорения, пусть сам роутер определяет как ему обрабатывать правила
- переработаны скрипты kvas-iface-add, kvas-iface-del, check_vpn
- скрипт adguard перевел на глобальные переменные
